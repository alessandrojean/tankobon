...
export interface PersonPictureFormProps {
...
export interface PersonPictureFormEmits {
  (e: 'update:picture', picture: Picture): void
}

const props = withDefaults(defineProps<PersonPictureFormProps>(), {
  currentImageUrl: undefined,
  disabled: false,
})

const emit = defineEmits<PersonPictureFormEmits>()
  ...
  <fieldset :disabled="disabled" class="grid grid-cols-1 sm:grid-cols-4 xl:grid-cols-5 gap-4">
    <div class="flex flex-col items-center gap-4">
      <FadeTransition>
        ...
        >
          <PaintBrushIcon class="w-10 h-10 text-gray-500 dark:text-gray-600" />
        </div>
        <img
          v-else
