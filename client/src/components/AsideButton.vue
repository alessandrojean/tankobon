...
<template>
  <a
    ...
    class="group flex items-center flex-nowrap text-sm rounded-lg w-full font-medium dark:focus-visible:ring-white/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-black" :class="[
      item.icon ? 'h-10' : 'h-9',
      active
  ...
  >
    ...
    >
      <Component
        :is="item.icon"
        class="w-6 h-6 motion-safe:transition-colors" :class="[
          active
    ...
    <span
      class="shrink-0 pl-3 box-border truncate motion-safe:transition-all" :class="[
