...
export interface PublisherPictureFormEmits {
  (e: 'update:picture', picture: Picture): void
}

const props = withDefaults(defineProps<PublisherPictureFormProps>(), {
  currentImageUrl: undefined,
  disabled: false,
})

const emit = defineEmits<PublisherPictureFormEmits>()
  ...
  <fieldset :disabled="disabled" class="grid grid-cols-1 sm:grid-cols-4 xl:grid-cols-5 gap-4">
    <div class="flex flex-col items-center gap-4">
      <FadeTransition>
        ...
        >
          <BuildingOffice2Icon class="w-10 h-10 text-gray-500 dark:text-gray-600" />
        </div>
        <img
          v-else
