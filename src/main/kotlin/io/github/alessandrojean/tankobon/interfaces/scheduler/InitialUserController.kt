...
class InitialUsersDevConfiguration {
  ...
  fun initialUsers(): List<TankobonUser> = listOf(
    TankobonUser("admin@example.org", "admin", isAdmin = true),
    TankobonUser("user@example.org", "user", isAdmin = false),
  )
}
...
class InitialUsersProdConfiguration {
  @Bean
  fun initialUsers(): List<TankobonUser> = listOf(
    TankobonUser("admin@example.org", RandomStringUtils.randomAlphanumeric(12), isAdmin = true),
  )
}