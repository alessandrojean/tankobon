...
class TankobonUserDaoTest(
  ...
  fun `it should persist multiple users when saving`() {
    tankobonUserDao.insert(TankobonUser("user1@example.org", "p", false, "User1"))
    tankobonUserDao.insert(TankobonUser("user2@example.org", "p", true, "User2"))
  ...
  fun `it should find an existing user by its id`() {
    val existing = TankobonUser("user1@example.org", "p", false, "User1")
